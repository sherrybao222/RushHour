...
class Node:
	def __init__(self, board, num_cars_each_level=None, params=None, parent=None):
		self.board = board # Board
		self.parent = parent # Node
		self.children = [] # list of Node
		if num_cars_each_level != None and params!=None:
			self.num_cars_each_level = num_cars_each_level
			self.value = - (np.sum(np.array(self.num_cars_each_level) 
					* np.array(params.weights))
					+ np.random.normal(loc=params.mu, scale=params.sigma))
		else:
			self.value = None
		self.maxchild = None
	# def __members(self):
	# 	return (self.board, self.parent, self.children, self.num_cars_each_level, self.value)
