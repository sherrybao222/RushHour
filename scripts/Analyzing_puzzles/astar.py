...
def make_Astar_all(heur=zeroh,calcF=make_fCalc(),is_stop=lambda x:False, lapse_rate=0,search_lapse=0,plan_correction_level=1,reconstruct_accuracy=100.0,search_limit=float('inf')):
    if lapse_rate>0:
        astar=make_Astar_all(heur,calcF,is_stop,0,search_lapse,plan_correction_level,reconstruct_accuracy,search_limit)
        def tmp_f(start):
            return lapsingAstar(start,astar,lapse_rate)
    else:
        def tmp_f(start):
            return Astar_all(start,heur,calcF,is_stop,search_lapse,plan_correction_level,reconstruct_accuracy,search_limit)
...
def Astar_all(start,heur=zeroh,calcF=make_fCalc(),is_stop=lambda x:False, search_lapse=0,plan_correction_level=1,reconstruct_accuracy=100.0, search_limit=float('inf')):
    ...
    while len(openList)>0:
        ...
        if n.is_goal():
            #print 'SOLVED'
            the_path = reconstruct_path(backtrace,n,plan_correction_level,reconstruct_accuracy)
            # print the_path
            if optlen_flag:
                optlen = len(the_path)
                # print optlen
                ...
                if len(the_path) == optlen:
                    print 'appended another'
                    the_paths.append(the_path)
                else:
                    print 'stop'
                    return the_paths,stats
        closed.add(n)
        succs = expand(n)
        for s in succs:
        ...
        stats['expanded']+=1
        if is_stop(stats):
            print 'stopping in Astar_all'
            n=select_state_from_open(openList)
            path_,stats_ = Astar_all(n,heur,calcF,is_stop,search_lapse,plan_correction_level,reconstruct_accuracy)
            stats['stops']+=1
