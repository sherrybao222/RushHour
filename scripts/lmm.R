...
# OLSexamp <- lm(humanlen ~ avgDepthSol + 
#                avgEdgeSol + avgMaxCycleSol + 
#                + avgNodeSol + avgcNodeSol + 
#                + avgnCycleSol + backMoveSol + 
#                + edgeRate + nodeRate + unsafeSol, 
#                data = train)
# display(OLSexamp)

# # generalized linear model
# MLexamp <- glm(humanlen ~ avgDepthSol + 
#                  avgEdgeSol + avgMaxCycleSol + 
#                  + avgNodeSol + avgcNodeSol + 
#                  + avgnCycleSol + backMoveSol + 
#                  + edgeRate + nodeRate + unsafeSol, 
...
# random intercept
MLexamp <- lmer(humanlen ~ avgDepthSol
                + avgEdgeSol 
                + avgMaxCycleSol
                + avgNodeSol 
                + avgcNodeSol 
...
plot(pred, test$humanlen,
     ...
     main='Pred vs True Log(humanlen) LMM Random Intercept', 
     cex=0.3)
...
step_result
# found model
MLexamp11 <- lmer(humanlen ~ avgDepthSol 
                + avgEdgeSol 
                + avgMaxCycleSol 
                + avgcNodeSol 
                + avgnCycleSol 
...
# random slope and intercept independent
MLexamp2 <- lmer(humanlen ~ avgDepthSol + (0 + avgDepthSol | sublist) 
                 + avgEdgeSol + (0 + avgEdgeSol | sublist)
                 + avgMaxCycleSol + (0 + avgMaxCycleSol | sublist)
                 + avgNodeSol + (0 + avgNodeSol | sublist)
                 + avgcNodeSol + (0 + avgcNodeSol | sublist)
...
plot(pred2, test$humanlen,
     xlab='Pred log(humanlen)', ylab='True log(humanlen)',
     main='Pred vs True Log(humanlen) LMM Rand Slope&Int indep', 
     cex=0.3)
lines(test$humanlen, test$humanlen, 
      lwd=2)
mtext(text=paste(c("R2 =", r.squaredGLMM(MLexamp2)[2]), collapse = " "), 
...
# random slope and intercept correlated
MLexamp3 <- lmer(humanlen ~ avgDepthSol + (1 + avgDepthSol | sublist) 
                 + avgEdgeSol + (1 + avgEdgeSol | sublist)
                 + avgMaxCycleSol + (1 + avgMaxCycleSol | sublist)
                 + avgNodeSol + (1 + avgNodeSol | sublist)
                 + avgcNodeSol + (1 + avgcNodeSol | sublist)
...
pred3 = predict(MLexamp3, test)
# scatter plot
plot(pred3, test$humanlen,
     xlab='Pred log(humanlen)', ylab='True log(humanlen)',
     main='Pred vs True Log(humanlen) LMM Rand Slope&Int corr', 
     cex=0.3)
lines(test$humanlen, test$humanlen, 
      lwd=2)
mtext(text=paste(c("R2 =", r.squaredGLMM(MLexamp3)[2]), collapse = " "), 
...
# line plots
plot(pred3, 
     main="Model Found LMM Rand Slopes & Int corr", 
     xlab="Trials", ylab="Log(humanlen)",
     type='l', col='blue', xlim=c(1400,1600), lwd=0.5)
lines(test$humanlen, type='l', col='red', xlim=c(1400,1600), lwd=0.5)
legend(1550, 14, legend=c("Target", "Predict"),
       col=c("red", "blue"), lty=1, cex=0.8,
       box.lty=0)
mtext(text=paste(c("R2 =", r.squaredGLMM(MLexamp3)[2]), collapse = " "), 
...
# random slope and intercept independent, single regressor
MLregr1 <- lmer(humanlen ~ avgDepthSol + (0 + avgDepthSol | sublist) 
                + (1 | sublist),
                data = train,
                REML=F)
MLregr2 <- lmer(humanlen ~ avgEdgeSol + (0 + avgEdgeSol | sublist)
                + (1 | sublist),
                data = train,
                REML=F)
MLregr3 <- lmer(humanlen ~ avgMaxCycleSol + (0 + avgMaxCycleSol | sublist)
                + (1 | sublist),
                data = train,
                REML=F)
MLregr4 <- lmer(humanlen ~ avgNodeSol + (0 + avgNodeSol | sublist)
                + (1 | sublist),
                data = train,
                REML=F)
MLregr5 <- lmer(humanlen ~ avgcNodeSol + (0 + avgcNodeSol | sublist)
                + (1 | sublist),
                data = train,
                REML=F)
MLregr6 <- lmer(humanlen ~ avgnCycleSol + (0 + avgnCycleSol | sublist)
                + (1 | sublist),
                data = train,
                REML=F)
MLregr7 <- lmer(humanlen ~ backMoveSol + (0 + backMoveSol | sublist)
                + (1 | sublist),
                data = train,
                REML=F)
MLregr8 <- lmer(humanlen ~ edgeRate + (0 + edgeRate | sublist)
                + (1 | sublist),
                data = train,
                REML=F)
MLregr9 <- lmer(humanlen ~ nodeRate + (0 + nodeRate | sublist)
                + (1 | sublist),
                data = train,
                REML=F)
MLregr10 <- lmer(humanlen ~ unsafeSol + (0 + unsafeSol | sublist)
                 + (1 | sublist),
                 data = train,
                 REML=F)
r.squaredGLMM(MLregr1)
